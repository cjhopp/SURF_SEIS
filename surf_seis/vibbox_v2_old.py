...
#        old_index = ''
#        for index,sta in max_sample.iteritems():
#            if old_index == index[:-1]:
#                max_sample = max_sample.drop(index)
#            old_index = index[:-1]
...
def vibbox_trigger(st, freqmin=1000, freqmax=15000, sta = 0.01, lta = 0.05, on = 1.3, off = 1, num = 10):
    ...
    for ii in range(len(trig)):
        if not 'SV.CTrig..' in trig[ii]['trace_ids']: # trow out CASSM shots
...
def vibbox_read(fname):
    ...
    A=2*VOLTAGE_RANGE*np.reshape(A, (len(A)/channels, channels))/(2^32)-VOLTAGE_RANGE
    ...
    time_to_first_full_second = np.where(A[:,clock_channel]>2e7)[0][0]-3
    ...
    if time_to_first_full_second < 0:
        time_to_first_full_second = np.where(A[50000:,clock_channel]>2e7)[0][0]-3
